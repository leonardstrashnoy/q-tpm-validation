...
- **Real-data validators do real, source-specific mappings** (both percentile-calibrated, since synthetic `PATHWAY_RULES` thresholds don't transfer to physical units; both write unavailable fields **NULL** rather than faking them). What's "real" differs by data product:
  - `qtpm_camels.py` reads Subfind HDF5 (`Group/`+`Subhalo/`, units 1e10 M⊙/h, type 4 = stars), KD-tree local density. Real: mass, stellar_mass, star_fraction → `analytical` (full), `global` (density, partial). NULL: all merger-history fields + their pathways (single snapshot, no tree).
  - `qtpm_bolshoi.py` reads Rockstar/consistent-trees `hlist_*.list` by **parsing the `#` header to bind columns by name** (`mvir`, `x/y/z`, `Acc_Rate*`, `scale_of_last_MM`). DM-only, so no stellar mass. Real: mass, local_density, recent_growth (specific accretion), last-MM timing → `value_driven`, `expedient` (growth+isolation), `global` (density, partial). NULL: stellar_mass/`analytical`, cumulative `major_mergers`/`ruling_guide`, `curvature`/`revisionist`.
  - Both reader paths are verified against constructed HDF5 / hlist fixtures (no real catalog is committed); when wiring real data, eyeball the first run.
